            ...
            <div class="col-3">
                <div class="card">
                    <div class="card-img ratio ratio-1x1">
                        <img src="{{ $order->image ? asset('images/kendaraan/' . $Kendaraan->image) : 'https://electricvehiclecouncil.com.au/wp-content/uploads/2023/02/4-1002x1024.png' }}"
                            alt="texzture 1" width="100%" height="100%" class="p-1 p-sm-3 rounded-3">
                    </div>
                </div>
            </div>
            <div class="col-9">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="text" id="name" class="form-control round" name="name"
                                    value="{{ old('name', $order->name) }}" disabled readonly>

                                <label for="merk">Merk Kendaraan</label>
                                <input type="text" id="merk" class="form-control round" name="merk"
                                    value="{{ old('merk', $order->merk) }}" disabled readonly>

                                <label for="plat_nomor">Plat Nomor Kendaraan</label>
                                <input type="text" id="plat_nomor" class="form-control round" name="plat_nomor"
                                    value="{{ old('plat_nomor', $order->plat_nomor) }}" disabled readonly>

                                <label for="harga">Harga</label>
                                <input type="number" id="harga" class="form-control round" name="harga"
                                    value="{{ old('harga', $order->harga) }}" disabled readonly>
                                <label for="warna">Warna Kendaraan</label>
                                <input type="text" id="warna" class="form-control round" name="warna"
                                    value="{{ old('warna', $order->warna) }}" disabled readonly>

                                <label for="kondisi">Kondisi Kendaraan</label>
                                <input type="text" id="kondisi" class="form-control round" name="kondisi"
                                    value="{{ old('kondisi', $order->kondisi) }}" disabled readonly>

                                <label for="tipe_kendaraan_id">Tipe Kendaraan</label>
                                <input type="text" id="tipe_kendaraan_id" class="form-control round"
                                    name="tipe_kendaraan_name" value="{{ $order->tipe_kendaraan->name }}" disabled readonly>

                                <label for="pemilik_id">Pemilik</label>
                                <input type="text" id="pemilik_id" class="form-control round" name="pemilik_id"
                                    value="{{ $order->pemilik->user->name }}" disabled readonly>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="card-body ">
                        ...
                        <form action="{{ route('order.store') }}" method="post">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="total_harga" class="form-control" placeholder="Total Price"
                            ...
                            <div class="form-group d-none">
                                ...
                                <input class="form-control round @error('kendaraan_id') is-invalid @enderror"
                                    name="kendaraan_id" id="kendaraan_id" value="{{ old('kendaraan_id', $order->id) }}"
                                    required>
    ...
    <script>
        document.getElementById('tanggal_perkiraan_kembali').addEventListener('change', function() {
            ...
            if (tanggal_perkiraan_kembali < tanggal_sewa) {
                var oneDayAfterSewa = new Date(tanggal_sewa.getTime() + (24 * 60 * 60 *
                    1000)); // Menambahkan satu hari
